...
class MusicCommand extends Command {
	constructor(bot, message, Util) {
		...
		this.endMusicReason = 'Stream is not generating quickly enough.'
		...
		if (url.match(this.urlYoutubePattern)) {
			...
			try {
				...
				try {
					...
					video = await youtube.getVideoByID(video[videoIndex - 1].id)
				} catch (err) {
	...
	stop() {
		...
		this.serverQueue.connection.dispatcher.end('Skip command has been used!')
		return undefined
	}
	...
	async chooseVideoHandler() {
		await this.message.channel.awaitMessage(msg => msg.content > 0 && msg.content < 11, {
	...
	async player(guild, song) {
		...
		const dispatcher = serverQueue.connection.playStream(stream, streamOptions)
			.on('end', reason => {
				if (reason === this.endMusicReason) console.log('Song Ended.')
				else console.log(reason)

				if(!serverQueue.songs[0]) {
					serverQueue.connection.disconnect()
					this.message.channel.send(':warning: There are no more songs in the queue!')
					return
				}
