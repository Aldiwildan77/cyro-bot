...
class MusicCommand extends Command {
	constructor(bot, message, Util) {
		...
		this.thirdCommand = Array.from(this.messageArray, (val, index) => {
			if (index >= 2) {
				return val
			}
		}).join(' ').trimLeft();
	...
	reply() {
		...
		switch (this.secondCommand) {
			case 'pl':
			case 'play':
				this.play(this.thirdCommand)
					.then(() => console.log('Music Played'))
					.catch(error => console.error('Error Play Music : ', error));
				break;
			case 'ps':
			case 'pause':
				this.pause()
					.then(() => console.log('Music Paused'))
					.catch(error => console.error('Error Pause Music : ', error));
				break;
			case 'rs':
			case 'resume':
				this.resume()
					.then(() => console.log('Music Resumed'))
					.catch(error => console.error('Error Resume Music : ', error));
				break;
			case 's':
			case 'skip':
				this.skip()
					.then(() => console.log('Music Skipped'))
					.catch(error => console.error('Error Skip Music : ', error));
				break;
			case 'st':
			case 'stop':
				this.stop()
					.then(() => console.log('Music Stopped'))
					.catch(error => console.error('Error Stop Music : ', error));
				break;
			case 'np':
			case 'now':
				this.nowPlaying()
					.then(() => console.log('Music are playing now'))
					.catch(error => console.error('Error Show Now Playing Music : ', error));
				break;
			case 'q':
			case 'queue':
				this.queue()
					.then(() => console.log('Music Queue'))
					.catch(error => console.error('Error Show Queue Music : ', error));
				break;
			default:
				break
		}
	...
	async videoHandler(video, message, voiceChannel, playlist = false) {
		...
		if (this.serverQueue) {
			...
			try {
				musicConstruct.connection = await voiceChannel.join();
